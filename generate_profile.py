...
USERNAME = "competitive7coder"

PORTRAIT_FILE = "portrait.txt"   # generated by photo_to_ascii.py

def load_portrait(theme_name):
    suffix = "dark" if theme_name == "dark" else "light"
    f = Path(__file__).parent / f"portrait_{suffix}.txt"
    if not f.exists():
        f = Path(__file__).parent / "portrait.txt"
    if f.exists():
        return f.read_text(encoding="utf-8").rstrip("\n").split("\n")
    return [f"[ portrait_{suffix}.txt missing ]"]


# (label, value, colour-key)  |  colour-key: key / val / accent / warn / muted
INFO = [
    ("__header__", "Protyush Ghorui", ""),
    ("__rule__", "", ""),
    ("Role", "Full Stack Developer", "val"),
    ("Edu", "B.Tech IT · MCKV Institute of Engineering (2027)", "val"),
    ("Focus", "Full Stack · FastAPI · React · ML Integration", "accent"),
    ("__blank__", "", ""),
    ("__section__", "~/stack", ""),
    ("Frontend", "Next.js · React.js · Tailwind CSS · Bootstrap 5", "val"),
    ("Backend", "FastAPI · Node.js · Django", "val"),
    ("Database", "PostgreSQL · MongoDB", "val"),
    ("Testing", "Jest", "val"),
    ("DevOps", "Docker · Cloudflare · Render · Vercel", "val"),
    ("Tools", "Git · GitHub · VS Code · Playwright", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/projects", ""),
    ("StreamHub", "OTT Platform with ML Engine", "warn"),
    ("MediQR", "Digital Healthcare Management Platform", "warn"),
    ("Kernel", "Personal Portfolio", "warn"),
    ("GSSoC'25", "Open-Source Contributor", "warn"),
    ("__blank__", "", ""),
    ("__section__", "~/highlights", ""),
    ("Solved", "300+ DSA problems on LeetCode", "val"),
    ("Mentored", "2 junior contributors in GSSoC'25", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/reach", ""),
    ("GitHub", "github.com/competitive7coder", "accent"),
    ("LinkedIn", "linkedin.com/in/protyush-ghorui-6121b0316", "accent"),
    ("Portfolio", "protyush.is-a.dev", "accent"),
    ("Mail", "ghorui.protyushraj@gmail.com", "accent"),
]
...
W, H = 980, 620
ART_X, ART_Y = 30, 86
ART_CW = 3.5          # forced char width (textLength keeps this exact in any font)
ART_LH = ART_CW * 1.72
...
def render(theme_name, colors, stats, ist_now):
    art_lines = load_portrait(theme_name)
